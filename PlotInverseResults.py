...
def plot_inverse_results(use_fwd_model, txt_string, unsupervised):
    ...
    if use_fwd_model:
        ...
        if np.any(ct_vals):
            ...
            if plot_ct_uncertainty:
                [1].fill_between(xvals-0.1, (1 - ct_vals) - ct_uncertainty, (1 - ct_vals) + ct_uncertainty,
                                 color='black', alpha=0.1)

    if plot_guess:
        [1].plot(xvals, 1 - initvec[0:NELEC], marker='o', color='purple', label='initial guess')
    ...
    if use_fwd_model:
        ...
        if plot_guess:
            [2].plot(xvals, initvec[NELEC:], marker='o', color='purple', label='initial guess')
