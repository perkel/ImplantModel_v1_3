...
def fig9_summary():
    ...
    for r_idx, res_ext in enumerate(r_vals):

        # get data
        thresh_err_summary, rpos_fit_vals, rpos_vals, rpos_err_summary, aaa_temp, dist_corr, dist_corr_p = read_inv_summary(res_ext)
        ...
        if r_idx == 0:
            ...
            axs1[0, 0].plot(thresh_err_summary[:, 0], thresh_err_summary[:, 1], 'o', color='blue')
    ...
    axs1[2, 0].set_xlabel('CT distance (mm)')
    axs1[2, 0].set_ylabel('Fit distance (mm)')
    axs1[2, 0].spines['top'].set_visible(False)
    axs1[2, 0].spines['right'].set_visible(False)
    axs1[2, 0].text(-0.2, label_ypos, 'D', size=20, weight='bold', transform=axs1[1, 1].transAxes)
